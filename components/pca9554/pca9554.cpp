...
void PCA9554Component::setup() {
  ...
  // pump-monitor override: PRESERVE the expander's current output/config
  // state instead of forcing all-inputs / all-low. The PCA9557 keeps its
  // outputs through an ESP soft reset (its /RESET is pulled up, not tied to
  // the ESP), so with this the ICE pump-loop relay stays energized across
  // OTA/reboots. Stock ESPHome briefly drops every output here, which on a
  // latching pump starter is a pump kill. Each switch's own setup then
  // (re)writes its restored state without a glitch.
  {
    uint8_t buf[2] = {0, 0};
    if (this->read_register(OUTPUT_REG * this->reg_width_, buf, this->reg_width_) == i2c::ERROR_OK) {
      this->output_mask_ = buf[0] | (buf[1] << 8);
    }
    if (this->read_register(CONFIG_REG * this->reg_width_, buf, this->reg_width_) == i2c::ERROR_OK) {
      // part: 1 = input; mask: 1 = output
      this->config_mask_ = (uint16_t) ~(buf[0] | (buf[1] << 8));
    }
    ESP_LOGI(TAG, "Preserved expander state: outputs=0x%02X outputs-configured=0x%02X",
             this->output_mask_, this->config_mask_);
  }
  // Read the inputs
  this->read_inputs_();
